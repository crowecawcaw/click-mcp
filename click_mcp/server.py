    ...
    def _execute_command(
        ...
        with contextlib.redirect_stdout(output):
            try:
                ...
                self._execute_command_with_context_chain(path_segments, args)
            except Exception as e:
                raise ValueError(f"Command execution failed: {str(e)}") from e

        return {"output": output.getvalue().rstrip()}

    def _execute_command_with_context_chain(
        self, path_segments: List[str], args: List[str]
    ) -> None:
        """
        Execute command with proper context inheritance.
        
        This implements Click's proper execution pattern to ensure parent contexts
        ...
        # Execute each level in the command hierarchy
        for i, cmd_name in enumerate(path_segments):
            is_final_command = (i == len(path_segments) - 1)
            
            # Get the command at this level
            cmd = current_group.get_command(parent_ctx, cmd_name)
            if cmd is None:
                raise ValueError(f"Command not found: {cmd_name}")
            
            # Determine arguments for this command level
            if is_final_command:
                # Final command gets the actual arguments
                cmd_args = args
            else:
                # Parent commands get empty arguments (they typically use options)
                cmd_args = []
            
            # Create context with proper parent relationship
            if parent_ctx is None:
                # Root command
                ctx = cmd.make_context(cmd_name, cmd_args)
            else:
                # Child command with parent context (enables context inheritance)
            ...
            with ctx:
                if is_final_command:
                    ...
                    # Execute parent command to set up context
                    cmd.invoke(ctx)
                    ...
                    if isinstance(cmd, click.Group):
                        ...
                        raise ValueError(f"'{cmd_name}' is not a group but has child commands")
